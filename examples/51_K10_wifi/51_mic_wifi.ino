 ...
 *
 * 按键交互:
 * - 按键 A: 短按=切换音频流+发送F2, 长按=发送Enter
 * - 按键 B: 短按=切换界面, 长按=连续发送Backspace
 * - A+B 同时 2 秒: 触发演示模式
 *
 * @date 2026-08-20
 */
...
#define ADC_KEY_CHANNEL ADC1_CHANNEL_1
#define ADC_SAMPLES     8       // 多次采样取平均，提高稳定性
#define ADC_DEBOUNCE_MS 50      // 消抖时间 50ms

// 电压阈值 (mV) - 根据实测值校准
// UP: 220Ω → ~37mV,  DOWN: 1kΩ → ~258mV,  LEFT: 4.7kΩ → ~1008mV,  RIGHT: 10kΩ → ~1573mV
#define VOLTAGE_UP_MIN      0
#define VOLTAGE_UP_MAX      150
#define VOLTAGE_DOWN_MIN    150
#define VOLTAGE_DOWN_MAX    600
#define VOLTAGE_LEFT_MIN    600
#define VOLTAGE_LEFT_MAX    1300
#define VOLTAGE_RIGHT_MIN   1300
#define VOLTAGE_RIGHT_MAX   2100
#define VOLTAGE_NONE_MIN    2100  // 未按键时接近 3.3V
...
static uint32_t  adc_press_time    = 0;
static uint32_t  adc_press_voltage = 0;  // 按下时的电压值（用于调试输出）
static uint32_t  adc_last_read     = 0;
// ADC 按键4双击检测 (进入演示模式)
static uint8_t   adc_key4_click_count = 0;          // 按键4点击次数
static uint32_t  adc_key4_last_release = 0;         // 按键4上次释放时间
static const uint32_t ADC_DOUBLE_CLICK_WINDOW_MS = 500;  // 双击时间窗口 500ms
...
// 界面管理
// 界面ID: 0=Status, 1=Token, 2=Project, 3=Inspo, 4=Profile, 5=AI
static uint8_t  current_screen = 5;  // 开机默认 AI Status
static bool     screen_dirty   = true;
// 界面切换循环顺序: 5(AI) -> 2(Project) -> 1(Token) -> 3(Inspo) -> 4(Profile) -> 0(Status) -> 回到5
static const uint8_t SCREEN_ORDER[6] = {5, 2, 1, 3, 4, 0};
...
static bool es7243e_init(uint8_t i2c_addr) {
    ...
    for (int i = 0; i < sizeof(init_seq) / sizeof(init_seq[0]); i++) {
        ...
        if (Wire.endTransmission() != 0) {
            Serial.printf("  ES7243E write FAILED at step %d\n", i);
            return false;
        }
...
static void adc_key_init() {
    // ADC1 不受 WiFi 影响，可与 ESP-NOW 共存
    adc1_config_width(ADC_WIDTH_BIT_12);
    adc1_config_channel_atten(ADC_KEY_CHANNEL, ADC_ATTEN_DB_11);  // 0-3.3V 量程
    Serial.println("ADC key initialized on GPIO2 (ADC1_CH1)");
}

static uint32_t adc_read_voltage_mv() {
    uint32_t sum = 0;
    for (int i = 0; i < ADC_SAMPLES; i++) {
        sum += adc1_get_raw(ADC_KEY_CHANNEL);  // 12-bit ADC (0-4095)
        delayMicroseconds(100);
    }
    uint32_t avg_raw = sum / ADC_SAMPLES;
    uint32_t voltage_mv = (avg_raw * 3300) / 4095;
    return voltage_mv;
}

static adc_key_t adc_get_key(uint32_t *out_voltage) {
    uint32_t voltage = adc_read_voltage_mv();
    if (out_voltage) *out_voltage = voltage;  // 返回电压值供调试

    // 电压从低到高: 按键2(~43mV) < 按键3(~261mV) < 按键4(~1017mV) < 按键1(~1576mV)
    if (voltage >= VOLTAGE_UP_MIN && voltage < VOLTAGE_UP_MAX) {
        return ADC_KEY_DOWN;     // ~43mV = 按键 2
    } else if (voltage >= VOLTAGE_DOWN_MIN && voltage < VOLTAGE_DOWN_MAX) {
        return ADC_KEY_LEFT;     // ~261mV = 按键 3
    } else if (voltage >= VOLTAGE_LEFT_MIN && voltage < VOLTAGE_LEFT_MAX) {
        return ADC_KEY_RIGHT;    // ~1017mV = 按键 4
    } else if (voltage >= VOLTAGE_RIGHT_MIN && voltage < VOLTAGE_RIGHT_MAX) {
        return ADC_KEY_UP;       // ~1576mV = 按键 1
    } else {
        return ADC_KEY_NONE;
    }
}

// handle_adc_key_event 移至 send_key 函数后面（需要引用演示模式变量）
...
static void switch_screen(uint8_t screen_num) {
    current_screen = screen_num % 6;  // 现在是 6 个界面
    screen_dirty = true;
...
static void handle_adc_key_event(adc_key_t key, bool is_long_press, uint32_t voltage) {
    if (key == ADC_KEY_NONE) return;

    const char* key_names[] = {"无", "1", "2", "3", "4"};
    Serial.printf("ADC 按键: %s (%s) [%d mV]\n", key_names[key],
                  is_long_press ? "长按" : "短按", voltage);

    // 按键功能映射（方案 C：混合实用）
    // 按键 1: 完全复用按键 A（短按=F2+切换音频流, 长按=Enter）
    // 按键 2: 短按=回车, 长按=Esc
    // 按键 3: 短按=删除, 长按=Ctrl+Backspace（删除整词）
    // 按键 4: 短按=Win键, 长按=Tab

    switch (key) {
        case ADC_KEY_UP:  // 按键 1 - 复用按键 A 的完整逻辑
        ...
        case ADC_KEY_DOWN:  // 按键 2
            if (!is_long_press) {
                Serial.println("  动作: 回车 (Enter)");
                send_key(0x28, "Enter");
            } else {
                Serial.println("  动作: Esc");
                send_key(0x29, "Esc");
            }
            break;

        case ADC_KEY_LEFT:  // 按键 3
            if (!is_long_press) {
                Serial.println("  动作: 删除 (Backspace)");
                send_key(0x2A, "Backspace");
            } else {
                Serial.println("  动作: Ctrl+Backspace (删除整词)");
                // Ctrl+Backspace 需要发送 modifier
                key_frame_t kf;
                kf.frame_type = FRAME_TYPE_KEY;
                kf.modifier = 0x01;  // Left Ctrl
                memset(kf.keycode, 0, 6);
                kf.timestamp = (uint16_t)(millis() & 0xFFFF);
                kf.seq_num = 0;
                ...
                Serial.println("Key sent: Ctrl+Backspace");
            }
            break;

        case ADC_KEY_RIGHT:  // 按键 4 - 切换屏幕（双击检测在 loop 中处理）
            if (!is_long_press) {
                Serial.println("  动作: 切换屏幕");
...
void loop() {
    ...
    {
        bool a_now = (digital_read(eP5_KeyA) == 0);
        bool b_now = (digital_read(eP11_KeyB) == 0);
    ...
    if (now - lastBtnB >= 10) {
        lastBtnB = now;
        bool b = (digital_read(eP11_KeyB) == 0);
    ...
    // --- ADC 分压四键检测 (消抖 + 长短按) ---
    if (now - adc_last_read >= ADC_DEBOUNCE_MS) {
        adc_last_read = now;
        uint32_t adc_voltage = 0;
        adc_key_t key_now = adc_get_key(&adc_voltage);

        if (key_now != ADC_KEY_NONE && prev_adc_key == ADC_KEY_NONE) {
            // 按键按下
            adc_press_time = now;
            adc_press_voltage = adc_voltage;
            prev_adc_key = key_now;
        } else if (key_now == ADC_KEY_NONE && prev_adc_key != ADC_KEY_NONE) {
            // 按键释放
            uint32_t duration = now - adc_press_time;
            bool is_long_press = (duration >= LONG_PRESS_MS);

            // 按键4双击检测 (短按才计数)
            if (prev_adc_key == ADC_KEY_RIGHT && !is_long_press) {
                if (now - adc_key4_last_release <= ADC_DOUBLE_CLICK_WINDOW_MS) {
                    ...
                    adc_key4_click_count = 1;  // 重新开始计数
                }
                adc_key4_last_release = now;
            } else {
                // 其他按键释放时重置按键4计数
                if (prev_adc_key != ADC_KEY_RIGHT) {
                    adc_key4_click_count = 0;
                }
            }

            handle_adc_key_event(prev_adc_key, is_long_press, adc_press_voltage);
            prev_adc_key = ADC_KEY_NONE;
        }
    }
